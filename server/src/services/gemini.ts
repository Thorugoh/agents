...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o audio para português do Brasil. Seja preciso e naatural na transcrição. Matenha a pontuação adequada e divida o texto em parágrafos.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  if (!response.text) {
    throw new Error('Failed generating answer using Gemini');
  }
